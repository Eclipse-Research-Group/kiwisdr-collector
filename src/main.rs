...
use url::Url;
use kiwisdr::{kiwi::{AMTuning, AgcConfig, Station}, KiwiSdrBuilder, KiwiSdrError, KiwiSdrSndClient};
...
async fn main() {
    ...
    for station in config.stations {
        log::info!("Connecting to {} at {}...", station.name.green(), station.endpoint);
        ...
        let task = tokio::spawn(async move {
            ...
            log::info!("Connected to {} at {}", station.name.green(), station.endpoint);
            if let Ok(mut kiwi_sdr) = client {
                ...
                log::info!("Tuning {} to {} kHz...", station.name.green(), station.frequency);
                kiwi_sdr.tune(Station::AM(AMTuning {
                ...
                kiwi_sdr.configure_agc(AgcConfig::new(false, false, -20, 6, 1000, 10)).await.unwrap();
                ...
                let mut writer = WavWriter::create(Path::new(&data_dir).join(format!("{}_{}.wav", station.name, station.frequency)), WavSpec {
                    channels: 1,
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        log::info!("{}: Sending keepalive", station_name.green());
